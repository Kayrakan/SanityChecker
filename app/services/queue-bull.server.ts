// Lazy BullMQ usage to avoid import-time failures in dev before deps/Redis exist
...
function buildConnection() {
  ...
  if (url) {
    ...
    return { url, maxRetriesPerRequest: null as any } as any;
  }
...
async function getQueue() {
  if (!queuePromise) {
    queuePromise = (async () => {
      ...
      return new Queue("jobs", { connection: bullConnection as any });
    })();
  }
  return queuePromise;
}
...
// Job names
export type JobName = "SCENARIO_RUN" | "DIGEST_EMAIL";

// Enqueue a scenario run using BullMQ.
// Creates a Run row first for idempotency, then enqueues with runId.
export async function enqueueScenarioRunBull(shopId: string, scenarioId: string, opts?: JobsOptions) {
  ...
  await q.add(
    "SCENARIO_RUN",
    { shopId, scenarioId, runId: run.id },
    {
      attempts: 5,
...
export async function enqueueDigestEmailBull(shopId: string, opts?: JobsOptions) {
  const q = await getQueue();
  await q.add(
    "DIGEST_EMAIL",
    { shopId },
