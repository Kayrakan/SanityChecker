...
import { defaultCountries, getTestAddress } from "./test-addresses.server";

export async function seedStarterScenarios(shopDomain: string, shopId: string) {
  ...
  const products: any[] = json?.data?.products?.nodes ?? [];
  const variants: { id: string; weight?: number }[] = products.flatMap((p) => p.variants?.nodes ?? []);
