...
export function listCountries(): { label: string; value: string }[] {
  return COUNTRIES.map(c => ({ label: c.name, value: c.code }));
}

export function getCountrySchema(code: string): CountrySchema | undefined {
  return COUNTRIES.find(c => c.code === code.toUpperCase());
}

export function getSubdivisions(code: string): Subdivision[] {
  return getCountrySchema(code)?.subdivisions ?? [];
}

export function listCountrySchemas(): CountrySchema[] {
  return COUNTRIES;
}
